...
QColor MidiChannel::colorByChannelNumber(ubyte number) {
    switch(number){
...
NoteOnEvent* MidiChannel::insertNote(ubyte note, int startTick, int endTick, ubyte velocity, MidiTrack *track){
    ProtocolEntry *toCopy = copy();
...
ubyte MidiChannel::progAtTick(int tick){
    if (_events->isEmpty())
        return 0;
