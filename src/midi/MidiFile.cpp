...
MidiFile::MidiFile(QObject *parent) : ProtocolEntry(parent) {
	...
	connect(instrumentTrack, SIGNAL(trackChanged()), this,
			SIGNAL(trackChanged()));
...
bool MidiFile::readMidiFile(QDataStream *content, QStringList *log) {
	...
	foreach (OnEvent *onevent, OffEvent::corruptedOnEvents()) {
		log->append("Warning: found OnEvent without OffEvent (line " + QString::number(
						onevent->line()) + ") - removing...");
...
bool MidiFile::readTrack(QDataStream *content, int num, QStringList *log) {
	...
	QString badHeader = "Error: Bad format in track header (Track " +
						QString::number(num) + ", Expected MTrk).";
	(*content) >> tempByte;
	if (tempByte != 'M') {
		log->append(badHeader);
		return false;
	...
	while (!endEvent) {
		...
		if (event && event->line() == MidiEvent::TEXT_EVENT_LINE) {
			TextEvent *textEvent = qobject_cast<TextEvent *>(event);
			if (textEvent) {
				if (textEvent->textType() == TextEvent::TrackNameTextEventType) {
	...
	// end of track
	(*content) >> tempByte;
	QString errorText = "Error: track " + QString::number(num) +
						"not ended as expected. ";
	if (tempByte != 0x00) {
	...
	if (!channel(18)->eventMap()->contains(0)) {
		log->append(
			"Warning: no TimeSignatureEvent detected at tick 0. Adding "
			"default value.");
	...
	if (!channel(17)->eventMap()->contains(0)) {
		log->append(
			"Warning: no TempoChangeEvent detected at tick 0. Adding "
			"default value.");
...
void MidiFile::calcMaxTime() {
	...
	for (int i = 0; i < events.length(); i++) {
		TempoChangeEvent *ev = qobject_cast<TempoChangeEvent *>(events.at(i));
		if (!ev) {
			continue;
		}
		int ticks = 0;
...
int MidiFile::tick(int ms) {
	...
	QList<MidiEvent *> events = channels[17]->eventMap()->values();
	TempoChangeEvent *event = Q_NULLPTR;
	...
	int startTick = qRound((ms - time) / event->msPerTick() + event->midiTime());
	return startTick;
}

int MidiFile::msOfTick(int tick,
...
int MidiFile::tick(int startms,
	...
	*eventList = new QList<MidiEvent *>;
	...
	int startTick = int((startms - time) / event->msPerTick() + event->midiTime());
...
int MidiFile::measure(int startTick,
	...
	*eventList = new QList<TimeSignatureEvent *>;
	...
	TimeSignatureEvent *event = Q_NULLPTR;
	int i = 0;
	int measure = 1;

	// find the startEvent and the firstTick
	for (; i < events.length(); i++) {
		TimeSignatureEvent *ev = qobject_cast<TimeSignatureEvent *>(events.at(i));
		if (!ev) {
...
void MidiFile::reloadState(ProtocolEntry *entry) {
	MidiFile *file = qobject_cast<MidiFile *>(entry);
	if (file) {
...
int MidiFile::tonalityAt(int tick) {
	...
	KeySignatureEvent *event = Q_NULLPTR;
	while (it != events->end()) {
		KeySignatureEvent *keySig = qobject_cast<KeySignatureEvent *>(it.value());
...
void MidiFile::meterAt(int tick, int *num, int *denum) {
	...
	while (it != meterEvents->end()) {
		TimeSignatureEvent *timeSig = qobject_cast<TimeSignatureEvent *>(it.value());
...
QList<int> MidiFile::quantization(int fractionSize) {
	int fractionTicks = qFloor((4 * ticksPerQuarter()) / qPow(2, fractionSize));

	QList<int> list;

	QList<MidiEvent *> timeSigs = timeSignatureEvents()->values();
	TimeSignatureEvent *last = Q_NULLPTR;
	foreach (MidiEvent *event, timeSigs) {
		TimeSignatureEvent *t = qobject_cast<TimeSignatureEvent *>(event);
...
void MidiFile::insertEventInChannel(int mChannel, MidiEvent *event, int tick)
{
	if (mChannel < 0 || mChannel > 18) {
		return;
	}
	channels[mChannel]-> insertEvent(event, tick);
}
