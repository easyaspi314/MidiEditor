...
void MidiFixer::addAgbCompatibleEvents(MidiFile *midiFile, int modType) {
    ...
    for (int currentChannel = 0; currentChannel < 16; currentChannel++) {
        ...
        QList<MidiEvent*> eventsToRemove = QList<MidiEvent*>();
        QMultiMap<int, MidiEvent*> eventsToAdd = QMultiMap<int, MidiEvent*>();
        ...
        it = eventsToAdd.constBegin();
        while (it != eventsToAdd.constEnd()) {
...
void MidiFixer::fixLoopCarryBack(MidiFile *midiFile) {
    ...
    QMultiMap<int, MidiEvent*>::const_iterator it16 = events16->constBegin();
    while (it16 != events16->constEnd()) {
        ...
        if (txtEvent) {   // if event is META
            if (txtEvent->textType() == TextEvent::MarkerTextEventType) {    // if event is Marker
                if (txtEvent->text() == L1("[") || txtEvent->text() == L1("loopStart")) {
                    if (txtEvent->text() == L1("loopStart"))
                        txtEvent->setText(_("["));
                    hasLoopStart = true;
                    loopStartTick = txtEvent->midiTime();
                } else if (txtEvent->text() == L1("]") || txtEvent->text() == L1("loopEnd")) {
                    if (txtEvent->text() == L1("loopEnd"))
                        txtEvent->setText(_("]"));
                    hasLoopEnd = true;
    ...
    for (int currentChannel = 0; currentChannel < 16; currentChannel++)
    {
        ...
        it = events->constFind(eventAtLoopStart);
        while (it != events->constEnd()) {
            \
            MidiEvent *event = it.value();
            // now all events get recorded on continously update the loop start state
