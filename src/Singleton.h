...
void Singleton<T>::init() {
	...
	if (singleton.inited) {
		CreateInstanceFunction createFunction = (CreateInstanceFunction)
												Singleton::create.load();
...
template<class T> QBasicAtomicPointer<T* (void)> Singleton<T>::create =
...
template<class T> QBasicAtomicPointer<void> Singleton<T>::tptr =
	Q_BASIC_ATOMIC_INITIALIZER(nullptr);
