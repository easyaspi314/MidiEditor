...
SettingsDialog::SettingsDialog(const QString &title,
                               #ifdef ENABLE_REMOTE
                               RemoteServer *server,
                               #endif
    ...
    connect(_listWidget, &QListWidget::currentRowChanged, this, &SettingsDialog::rowChanged);
    ...
    connect(ok, &QPushButton::clicked, this, &SettingsDialog::submit);

    buttonLayout->setColumnStretch(0, 1);
    layout->addWidget(buttonBar, 2, 0, 1, 1);

    // add content
    addSetting(new GuiSettingsWidget(central));
    addSetting(new MidiSettingsWidget(central));
    addSetting(new AdditionalMidiSettingsWidget(central));
#ifdef ENABLE_REMOTE
    addSetting(new RemoteSettingsWidget(server, central));
#endif
    addSetting(new UpdateSettingsWidget(central));
}
