...
AboutDialog::AboutDialog(QWidget *parent) : QDialog(parent) {
    ...
    QLabel *title = new QLabel(L1("<h1>") % QApplication::applicationName() % L1("</h1>"), this);
    layout->addWidget(title, 0, 1, 1, 2);
    title->setStyleSheet("color: black");

    QLabel *version = new QLabel(L1("Version: ") % L1(GIT_TAG) % L1(" (") %
                                 QSysInfo::buildCpuArchitecture() % L1(GIT_DATE) % L1(")"), this);
